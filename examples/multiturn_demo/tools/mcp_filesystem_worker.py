...
from multiturn_demo.tools.MCP_io_struct import MCPRequest, MCPRequestType, MCPResponse, MCPSessionState
...
class MCPFilesystemClientWorker(ToolWorker):
    ...
    async def session_start(self, rollout_request) -> bool:
        session_id = rollout_request['session_id']
        session = await self._get_or_create_session(session_id, f'/projects/{session_id}')
        if not session:
            response_dict = {
                "success": False,
        ...
        await self.output_channel.put(response_dict, key=session_id, async_op=True).async_wait()

    async def session_end(self, rollout_request) -> bool:
        session_id = rollout_request['session_id']
    ...
    async def session_execute(self, rollout_request) -> bool:
        session_id = rollout_request['session_id']
        tool_name = rollout_request['tool_name']
        tool_args = rollout_request['tool_args']
        request_work = MCPRequest(
            ...
            metadata={
                ...
                "mount_dir": f'/projects/{session_id}',
            },
        )
    ...
    async def _process_requests(self):
        ...
        while self.is_running:
            try:
                ...
                print(f"zcy_dbg: MCPFilesystemClientWorker: run: {rollout_request}")
                request_type = rollout_request['request_type']
                if request_type == 'session_start':
                    asyncio.create_task(self.session_start(rollout_request))
                elif request_type == 'session_end':
                    asyncio.create_task(self.session_end(rollout_request))
                elif request_type == 'session_execute':
                    asyncio.create_task(self.session_execute(rollout_request))
                else:
