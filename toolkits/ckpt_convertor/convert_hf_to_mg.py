                    ...
                    for i, (ckpt_key, ckpt_value) in enumerate(all_ckpts.items()):
                        if convert_config.te_ln_add_extra_state is not None:
                            if convert_config.te_ln_add_extra_state == 'none':
                                extra_state = None
                            elif convert_config.te_ln_add_extra_state == 'tensor_pickle_none':
                                import pickle
                                state_serialized = bytearray(pickle.dumps(None))
                                extra_state = torch.frombuffer(state_serialized, dtype=torch.uint8)
                            elif convert_config.te_ln_add_extra_state == 'tensor_0_dim':
                                extra_state = torch.tensor([])
                            else:
                                assert False, f"te_ln_add_extra_state only avail in [None, 'none', 'tensor_pickle_none', 'tensor_0_dim']"
                                ...
                                for key_name in keys_to_check:
                                    if key_name.startswith("decoder.") and (
                                        ...
                                        if extra_state_key not in model_dict:
                                            extra_states_to_add[extra_state_key] = extra_state
...
def convert_hf_to_mg(
    ...
    assert (
        convert_config.te_ln_add_extra_state in [None, 'none', 'tensor_pickle_none', 'tensor_0_dim']
