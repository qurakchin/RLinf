...
This is intentionally the only module under ``rlinf.envs.real.yam`` that
...
class I2RTYamFollower(_I2RTYamBackend):
    ...
    def __init__(self, device_config: Any) -> None:
        ...
        # the second CAN chain is still opening.  Calibration tooling may opt
        # into zero-gravity mode via ``device_config.zero_gravity_mode``.
        super().__init__(
            device_config,
            zero_gravity_mode=bool(getattr(device_config, "zero_gravity_mode", False)),
        )
...
class I2RTYamLeader(_I2RTYamBackend):
    ...
    def __init__(self, device_config: Any) -> None:
        ...
        self._position_kp: np.ndarray | None = None
        self._position_kd: np.ndarray | None = None
        self._feedback_kp: np.ndarray | None = None
        self._feedback_kd: np.ndarray | None = None
    ...
    def connect(self) -> None:
        ...
        native_kd = np.asarray(info["kd"], dtype=np.float64).reshape(-1)
        if native_kd.shape != (NUM_ARM_JOINTS,):
            raise RuntimeError(
                f"YAM leader kd must have shape (6,), got {native_kd.shape}"
            )
        scale = float(self.device_config.bilateral_kp)
        self._position_kp = native_kp.copy()
        self._position_kd = native_kd.copy()
        self._feedback_kp = native_kp * scale if scale > 0 else np.zeros(NUM_ARM_JOINTS)
        self._feedback_kd = np.zeros(NUM_ARM_JOINTS)
        self._set_feedback_gains(robot)
    ...
    def command_feedback(self, follower_joints: np.ndarray) -> None:
        ...
        robot = self._require_robot()
        self._set_feedback_gains(robot)
        robot.command_joint_pos(joints.copy())

    def command(self, joints: np.ndarray) -> None:
        """Drive leader joints with the SDK's position-control gains."""
        joints = np.asarray(joints, dtype=np.float64).reshape(NUM_ARM_JOINTS)
        if not np.all(np.isfinite(joints)):
            raise ValueError("refusing non-finite YAM leader command")
        robot = self._require_robot()
        if self._position_kp is None or self._position_kd is None:
            raise RuntimeError("YAM leader position gains are not initialized")
        robot.update_kp_kd(
            kp=self._position_kp.copy(),
            kd=self._position_kd.copy(),
        )
        robot.command_joint_pos(joints.copy())
    ...
    def release_feedback(self) -> None:
        ...
        if self._robot is not None:
            self._set_feedback_gains(self._robot)
            self._robot.enter_gravity_comp_idle()

    def _set_feedback_gains(self, robot: Any) -> None:
        if self._feedback_kp is None or self._feedback_kd is None:
            raise RuntimeError("YAM leader feedback gains are not initialized")
        robot.update_kp_kd(
            kp=self._feedback_kp.copy(),
            kd=self._feedback_kd.copy(),
        )
