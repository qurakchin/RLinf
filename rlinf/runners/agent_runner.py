...
class AgentRunner:
    ...
    def __init__(
        ...
        self.agent_loop_channel = Channel.create("AgentLoop")
    ...
    def init_workers(self, init_agent_loop=True):
        ...
        # Init workers
        self.generate.init_worker().wait()
        self.actor.init_worker().wait()
        if self.has_dedicated_inference:
            self.inference.init_worker().wait()
        self.reward.init_worker().wait()
        if init_agent_loop:
            self.agent_loop.init_worker(self.generate_input_channel, self.generate_output_channel).wait()
    ...
    def _sync_weights(self):
        ...
        self.generate.sync_model_from_actor().wait()
    ...
    def run(self):
        ...
        self.generate.rollout(self.generate_input_channel, self.generate_output_channel)
        for _ in epoch_iter:
            for batch in self.train_dataloader:
                with self.timer("step"):
                    ...
                    rollout_handle: Handle = self.agent_loop.run_agentloop_rollout(
                        ...
                        output_channel=self.agent_loop_channel,
                    )

                    # Rewards
                    reward_handle: Handle = self.reward.compute_rewards(
                        input_channel=self.agent_loop_channel,
